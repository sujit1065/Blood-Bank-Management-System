...
import in.co.blood.bank.model.RoleModel;
...
public class LoginCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (id > 0) {
			...
			ServletUtility.forward(BBMView.LOGIN_VIEW, request, response);
			return;
		}
		if (session.getAttribute("user") != null) {
			ServletUtility.redirect(BBMView.WELCOME_CTL, request, response);
			return;
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_SIGN_IN.equalsIgnoreCase(op)) {
			...
			ServletUtility.redirect(BBMView.USER_REGISTRATION_CTL, request, response);
		return;
		}
		ServletUtility.forward(getView(), request, response);
